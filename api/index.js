...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── 슬랙 피드백 ── NEW
    else if (path==='slack' && method==='POST') result = await sendSlack(body);
...
async function sendSlack(body) {
  ...
  if(!webhookUrl) throw new Error('SLACK_WEBHOOK_URL 환경변수가 설정되지 않았습니다.');

  const emoji = { '기능요청':'💡', '버그신고':'🐛', '기타':'💭' }[body.type] || '💬';
  const text = `${emoji} *[맛집 지도 피드백] ${body.type}*\n\n*작성자:* @${body.nick}\n*내용:*\n${body.text}\n\n_${new Date().toLocaleString('ko-KR')}_`;
  ...
  if(!r.ok) throw new Error('Slack 전송 실패');
  return { success: true };
}
